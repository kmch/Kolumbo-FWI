...
from fwipy.project.files.text.submit import PbsFile


#FIXME REPLACE WITH CONTIGUOUS STRING

proj_path_thomas = 'FIXME' # used by proj.inp.rsync


@traced
@logged
class PbsFileThomas(PbsFile):
  """
  FIXME: NOW IT'S CX1
  Notes
  -----
  ...
  def _find_optimal_resources(self, optimize='idle', **kwargs):
    from fwipy.numeric.generic import divisors_of, decimal
  ...
  def _write_head(self, f, **kwargs):
    ...
    **kwargs : keyword arguments (optional)
    ...
    f.write('# For more explanation, see help(fwipy.project.files.runfiles.PbsFile).\n')
    f.write('#\n')
    f.write('##\n\n')
