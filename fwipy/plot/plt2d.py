...
from fwipy.plot.generic import set_xlabels
...
def compare_2d(A1, A2, **kwargs):
  ...
  if mode == 'interleave':
    from fwipy.ndat.arrays import interleave_arrays, Arr3d
...
def plot_image(image, widgets=False, center_cmap=False, cbar=True, **kwargs):
  ...
  if spect is not None:
    from fwipy.numeric.fourier import dft_freqs
...
def plot_wiggl(image, **kwargs):
  ...
  from fwipy.plot.plt1d import plot_1d
...
def _center_around_zero(minn, maxx, **kwargs): #NOTE
  ...
  ----------
  minn, maxx : float
    Extreme value of the image tfwipy.plot.
