"""
Format-independent file objects.
...
from fwipy.generic.parse import kw, del_kw, exten, strip, path_leave
from fwipy.generic.system import bash, exists
from fwipy.project.files.generic import ArrayProjFile
from fwipy.ioapi.generic import CsvFile, read_txt
from fwipy.ioapi.fw3d import TtrFile
...
class DataFile(ArrayProjFile):
  ...
  def read(self, **kwargs):
    ...
    self.array = Data(super().read(**kwargs))
    return self.array
@logged
class SynDataFile(DataFile):
  ...
  def _get_phase(self, freq, overwrite=True, **kwargs):
    ...
    if (not freq in self.phase) or overwrite:
      from fwipy.dsp.phase import first_breaks, extract_phase, wrap_phase
      from fwipy.numeric.generic import rms    
