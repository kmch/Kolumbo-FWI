...
from fwipy.project.files.generic import BinaryProjFile, ArrayProjFile
from fwipy.project.files.datalike.generic import DataFile, SynDataFile, ObsDataFile
from fwipy.project.files.text.hed import HedFile
from fwipy.project.files.other.index import IndexFile
...
class DataFileSgy(DataFile, SgyFile):
  ...
  def split_OLD(self, sid=None, lid=None, **kwargs):
    ...
    """
    from fwipy.ioapi.segy import split_sgy
...
class SynDataFileSgy(DataFileSgy, SynDataFile):
  ...
  def compare(self, **kwargs):
    ...
    This assumefwipy.plotting lines.
  ...
  def compare_phase_all_shots(self, **kwargs):
    ...
    for sid, synth in self.gather.items():
      for freq in [2, 3, 4]:
        ...
        plt.subplot(1,3,1)
        outseifwipy.plot_phase(freq=freq)
        plt.gca().set_aspect('equal')

        plt.subplot(1,3,2)
        syntfwipy.plot_phase(freq=freq)
        plt.gca().set_aspect('equal')

        plt.subplot(1,3,3)
        syntfwipy.plot_phase(freq=freq, subtract=outseis)
        plt.gca().set_aspect('equal')
...
class ObsDataFileSgy(DataFileSgy, ObsDataFile):
  ...
  def _get_phase(self, freq, **kwargs):
    ...
    This assumefwipy.plotting gathers.
...
class RawSignFile(DataFileSgy):
  ...
  def create(self, wavelet, **kwargs):
    ...
    if not isinstance(wavelet, str):
      ...
      if wavelet == 'ricker':
        from fwipy.numeric.funcs import ricker
