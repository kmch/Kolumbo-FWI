...
from fwipy.generic.parse import kw, del_kw, exten, strip
from fwipy.generic.system import bash, exists
from fwipy.project.files.generic import TextProjFile
from fwipy.seismic.srcrec import SRs, PointSR, Sources, Receivers
...
class SRFile(TextProjFile):
  ...
  def create(self, dictio, **kwargs):
    ...
    dictio : dictionary
      Dictionary with items:
        ID: [x, y, z]
    """
    from fwipy.ioapi.fw3d import save_pgy
    #from fwipy.ioapi.segy import save_geo
  ...
  def read(self, extend=False, unit='node', **kwargs):
    ...
    io = self.proj.io
    if io == 'sgy':
      from fwipy.ioapi.segy import read_geo
      ...
      from fwipy.ioapi.fw3d import read_pgy
