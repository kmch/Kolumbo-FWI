...
def shift_to_zero(A, fraction=0.001, **kwargs): # DEL BECAUSE fraction IS ARBITRARY?
  """
  """
  from fwipy.dsp.phase import first_breaks
...
def extract_wavelet(fname, proj_name, tend=1.5, ntaper=50, **kwargs):
  ...
  from fwipy.dsp.su import su_process, su_mute, su_filter_full
  from fwipy.project.t import ProjSynVsObs
...
def read_coords(fname, **kwargs):
  ...
  from fwipy.generic.parse import path_leave, path_extract
  name = path_leave(fname)
  path = path_extract(fname)
  sgy = SgyFile(name, path)
  
  h = sgy.read_header(**kwargs)
...
def align(A, i_target, **kwargs):
  ...
  """
  from fwipy.dsp.generic import xcorr
...
def limit_offsets(fnames, offset_min, offset_max, output_path=None, **kwargs):
  ...
  """
  from fwipy.generic.parse import path_leave, strip, extend_fname
  from fwipy.ioapi.su import suwind
